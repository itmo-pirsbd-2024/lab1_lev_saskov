...
public class StringEncoder implements DataEncoder<String> {
    ...
    public DecodeResult<String> decode(byte[] bytes, int from) {
        var strSizeDecode = intCoder.decode(bytes, from);
        var str = new String(bytes, from + strSizeDecode.bytesCount(), strSizeDecode.result(), charset);
    ...
    public DecodeResult<String> decode(ByteBuffer byteBuffer, int from) {
        var strSizeDecode = intCoder.decode(byteBuffer, from);
