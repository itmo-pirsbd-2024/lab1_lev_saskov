...
public class VarLongEncoder {
    ...
    public LongDecodeResult decode(byte[] encodedBytes, int from) {
        return decode(from, encodedBytes.length, i -> encodedBytes[i]);
    }

    public LongDecodeResult decode(ByteBuffer byteBuffer, int from) {
        return decode(from, byteBuffer.limit(), byteBuffer::get);
    }

    public LongDecodeResult decode(byte[] bytes) {
        return decode(bytes, 0);
    }

    public LongDecodeResult decode(ByteBuffer byteBuffer) {
        return decode(byteBuffer, 0);
    }

    private LongDecodeResult decode(int from, int limit, PositionGetter positionGetter) {
        long result = 0;
        byte shift = 0;

        int bytesCount;
        for (bytesCount = 0; bytesCount < limit; bytesCount++) {
            byte byteValue = positionGetter.get(bytesCount + from);
        ...
        return new LongDecodeResult((result >> 1) ^ (-(result & 1)), bytesCount);
    }

    public int putToList(List<byte[]> bytesList, long toEncode) {
        byte[] result = encode(toEncode);
        bytesList.add(result);
        return result.length;
    }

    public record LongDecodeResult(long result, int bytesCount) {
    }
